...
void autocomplete (const Arg *arg)
{
	...
	if (active == 0) {
		...
		char stbuffile_template[] = "/tmp/stXXXXXX";
		int stbuffile_fd = mkstemp(stbuffile_template);
		if (stbuffile_fd == -1) {
			perror("mkstemp");
			exit(EXIT_FAILURE);
		}
		stbuffile = stbuffile_template;

		FILE *stbuf = fdopen(stbuffile_fd, "w");
		if (!stbuf) {
			perror("fdopen");
			close(stbuffile_fd);
			exit(EXIT_FAILURE);
